...
public:
    ...
    bool operator()(const T *camera_extrinsics,
                    // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
                    const T *camera_intrinsics,
                    // внутренние калибровочные параметры камеры: [5] = {k1, k2, f, cx, cy} (одни и те же для всех кадров, т.к. снято на одну и ту же камеру)
        ...
        T locNoRot[3];
        for (int i = 0; i < 3; ++i) { locNoRot[i] = point_global[i] - camera_extrinsics[i]; }
        T locals[3];
        ceres::AngleAxisRotatePoint(camera_extrinsics + 3, locNoRot, locals);
        for (int i = 0; i < 2; ++i) { locals[i] /= locals[2]; }

#if ENABLE_INSTRINSICS_K1_K2
        // k1, k2 - коэффициенты радиального искажения (radial distortion)
        T r = ceres::pow(locals[0], 2) + ceres::pow(locals[1], 2);
        T k1 = camera_intrinsics[0];
        T k2 = camera_intrinsics[1];
        T c = (T)1.0 + k1 * r + k2 * r * r;
        locals[0] *= c;
        locals[1] *= c;
#endif
        ...
        locals[0] += camera_intrinsics[3];
        locals[1] += camera_intrinsics[4];
        // Теперь по спроецированным координатам не забудьте посчитать невязку репроекции
        residuals[0] = locals[0] - observed_x;
        residuals[1] = locals[1] - observed_y;
        return true;
    }
...
void runBA(std::vector<vector3d> &tie_points,
    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            ceres::CostFunction *keypoint_reprojection_residual = new ceres::AutoDiffCostFunction<ReprojectionError,
                        2,
                        // количество невязок (размер искомого residual массива переданного в функтор, т.е. размерность искомой невязки, у нас это dx, dy (ошибка проекции по обеим осям)
                        6, 5, 3>
                    // число параметров в каждом блоке параметров, у нас три блок параметров (внешние параметры камеры[6], внутренние параметры камеры[5] и 3D точка)
            ...
            if (!track.disabled) {
                ...
                // если мы не передали невязку в ceres-solver, то за его lifetime ответственны все еще мы
            }
        }
    }
    ...
    std::cout << "After BA tie poits: " << to_percent(n_old_outliers, tie_points.size()) << "% old + " <<
            to_percent(n_new_outliers, tie_points.size()) << "% new = " << to_percent(
                n_old_outliers + n_new_outliers, tie_points.size()) << "% total outliers" << std::endl;
    ...
    for (auto ptr: reprojection_residuals_for_deletion) {
        delete ptr;
        // т.к. мы не отдали указатель в ceres-solver - мы ответственны за его lifetime - надо самим деаллоцировать
    }
}
