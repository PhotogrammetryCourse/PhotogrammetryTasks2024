...
// При downscale мы уменьшаем плоскость изображения в downscale раз, кажется, что для того чтобы модель камеры оставалась той же самой
// мы должны усеньшить фокальную длину в том же соотношении иначе область обзора камеры поменяется
...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        x -= camera_extrinsics[0];
        y -= camera_extrinsics[1];
        ...
        T tmp_point[3] = {x, y, z};
        ceres::AngleAxisRotatePoint(rotation, tmp_point, tmp_res);
        x = tmp_res[0];
        y = tmp_res[1];
        z = tmp_res[2];
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA) {
                ...
                for (size_t cj = 0; cj < track.img_kpt_pairs.size(); ++cj) {
                    if (ci != cj) {
                        ...
                        phg::decomposeUndistortedPMatrix(R2, camera_origin2, cameras[camera_id]);
                        vector3d track_in_camera = R * (track_point - camera_origin);
                        vector3d track_in_camer2 = R2 * (track_point - camera_origin2);
                        
                        double cosine = cos(track_in_camera.dot(track_in_camer2) / (cv::norm(track_in_camera) * cv::norm(track_in_camer2)));
                        double angle;
                        if (cosine > 1.0) {
                            angle = 0.0;
                        }
