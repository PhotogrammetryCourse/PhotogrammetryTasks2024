...
// включите Bundle Adjustment (но из любопытства посмотрите как ведет себя реконструкция без BA например для saharov32 без BA)
#define ENABLE_BA 1

// когда заработает при малом количестве фотографий - увеличьте это ограничение до 100 чтобы попробовать обработать все фотографии (если же успешно будут отрабаывать только N фотографий - отправьте PR выставив здесь это N)
...
#define INTRINSIC_K1_K2_MIN_IMGS 15 // начиная со скольки камер начинать оптимизировать k1, k2
...
public:
    ...
    bool operator()(const T *camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        const T translation[3] = {camera_extrinsics[0], camera_extrinsics[1], camera_extrinsics[2]};

        // rotation[3] - angle-axis rotation, поворачиваем точку point->p (чтобы перейти в локальную систему координат камеры)
        const T rotation[3] = {camera_extrinsics[3], camera_extrinsics[4], camera_extrinsics[5]};
        ...
        T point[3];
        ceres::AngleAxisRotatePoint(rotation, point_translated, point);
        point[0] /= point[2];
        point[1] /= point[2];
        point[2] = (T)1.0;
        ...
        point[0] *= (T)1 + k1 * r + k2 * r * r;
        point[1] *= (T)1 + k1 * r + k2 * r * r;
#endif
