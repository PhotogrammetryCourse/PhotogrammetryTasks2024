...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        T translations[3] = { camera_extrinsics[0], camera_extrinsics[1], camera_extrinsics[2] };
        ...
        T pnt[3] = { point_global[0] - translations[0], point_global[1] - translations[1], point_global[2] - translations[2] };
        T pnt1[3];
        ceres::AngleAxisRotatePoint(rotation, pnt, pnt1);
        std::swap(pnt, pnt1);
        pnt[0] /= pnt[2];
        pnt[1] /= pnt[2];
        pnt[2] /= pnt[2];
        ...
        T k1_ = camera_intrinsics[0];
        T k2_ = camera_intrinsics[1];
        T r = pnt[0] * pnt[0] + pnt[1] * pnt[1];
        pnt[0] *= T(1) + k1_ * r + k2_ * r * r;
        pnt[1] *= T(1) + k1_ * r + k2_ * r * r;
#endif
        ...
        residuals[0] = pnt[0] - observed_x;
        residuals[1] = pnt[1] - observed_y;
        return true;
...
//http://ceres-solver.org/nnls_solving.html
    ...
    calib.cx_ = camera_intrinsics[3] - calib.width() / 2.0;
    calib.cy_ = camera_intrinsics[4] - calib.height() / 2.0;
