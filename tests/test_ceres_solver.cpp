...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        residual[0] = (samplePoint[0] * line[0] + samplePoint[1] * line[1] + line[2]) / ceres::sqrt(line[0] * line[0] + line[1] * line[1]);
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    const double ideal_line[3] = {1, -2.0, 200.0}; // 0.5*x - y + 100 = 0
    ...
    for (size_t i = 0; i < n_points; ++i) {
        ...
        if (i < n_points - n_points_outliers) {
            ...
            if (shift_distance > 3 * sigma) {
                shift_distance = 3 * sigma;
            }
            if (shift_distance < -3 * sigma) {
                shift_distance = 3 * sigma;
            } // we don't want not outliers to become outliers
    ...
    if (abs(line_params[0]) > 1e-4) {
        line_params[2] /= line_params[0];
        line_params[1] /= line_params[0];
        line_params[0] /= line_params[0];
    } else if (abs(line_params[1]) > 1e-4) {
        line_params[2] /= line_params[1];
        line_params[0] /= line_params[1];
        line_params[1] /= line_params[1];
    } else throw std::runtime_error("both line A, B are close to 0");

    std::cout << "Found line: (a=" << line_params[0] << ", b=" << line_params[1] << ", c=" << line_params[2] << ")" << std::endl;

    double threshold = 1.0 / n_points * std::max(std::abs(ideal_line[0]), std::max(std::abs(ideal_line[1]), std::abs(ideal_line[2])));
    ...
    ASSERT_GT(inliers_fraction, 0.99 - 1e-8); // TODO 8 раскоментируйте, почему эта проверка падает? как поправить?
    // fixed the strict inequality here and in the similar line below
    ...
    if (outliers_fraction == 0 || use_huber) {
        ...
        ASSERT_GT(inliers_fraction, 0.99 - 1e-8);
...
void evaluateLine(const std::vector<double_2> &points, const double* line,
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (dist <= 3 * sigma + 1) { // +1 as we can find the line with some inaccuracy
            ++inliers;
...
TEST (CeresSolver, FitLineNoiseAndOutliers) {
    const double sigma = 1.0;
    const double outliers_fraction = 0.01; // 1% outliers
