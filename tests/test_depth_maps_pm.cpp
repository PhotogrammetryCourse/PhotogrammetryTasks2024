...
TEST(test_depth_maps_pm, SingleDepthMap) {
    ...
    size_t ci = 4;            // строим карту глубины для третьей камеры (индексация с нуля)
    size_t cameras_limit = 9; // учитывая первые пять фотографий датасета, т.е. две камеры слева и две камеры справа
...
TEST(test_depth_maps_pm, AllDepthMaps) {
    ...
    for (size_t from = 0; from + to_shift <= full_dataset.ncameras; ++from) {
        ...
        std::string tie_points_filename = std::string("data/debug/test_depth_maps_pm/") + getTestName() + "/" DATASET_DIR "/all_points_" + to_string(ndepth_maps) + ".ply";
