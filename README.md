...
Реализуйте SIFT в ```src/phg/sift/sift.cpp```:
 ...
 1) При установке библиотеки OpenCV следуем интсрукциям (скачать zip-архив с версией 4.5.1, запустить cmake, make, install), тогда будет автоматически создан файл библиотеки `.so`, который переместится в нужный каталог с библиотеками (после этого при сборке проекта, библиотека будет сама находиться).
 2) А вот include-файлы библиотеки opencv, например, VScode автоматически не находит, для этого можно сделать следующее: 
    * Найти директорию с этими файлами:
        ```
        find . -type d -name opencv2
        ```
        эту команду я запускал из корневой директори `/`, эта команда ищет папку `opencv2`, где и лежат include.
    * После этого найденный путь до каталога `opencv2` явно добавить в `include path` в настройках VScode.
3) Для сборки уже самого проекта дз (без всяких настроек в среде разработки), можно воспользоваться командной строкой:

    ```
    cmake && make
    ```
    Этак команда создаст исполняемый файл `test_sift`, который уже можно запускать:
    ```
    ./test_sift
    ```
    После запуска появятся результаты выполнения тестов.
