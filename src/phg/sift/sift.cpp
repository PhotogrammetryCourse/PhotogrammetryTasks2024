...
#define DESCRIPTOR_SAMPLES_N       4 // 4x4 замера (направление градиентов) для каждой гистограммы дескриптора (всего гистограмм 4х4) итого 16х16 замеров
...
void phg::SIFT::detectAndCompute(const cv::Mat &originalImg, std::vector<cv::KeyPoint> &kps, cv::Mat &desc) {
    ...
    buildPyramids(img, gaussianPyramid, DoGPyramid);  // по картинке img -> строим пирамиду Гауссианов и их разность

    findLocalExtremasAndDescribe(gaussianPyramid, DoGPyramid, kps, desc);    // по пирамидам получаем вектор kps (ключевые точки) и desk - вектор их дескрипторов
}

void phg::SIFT::buildPyramids(const cv::Mat &imgOrg, std::vector<cv::Mat> &gaussianPyramid, std::vector<cv::Mat> &DoGPyramid) {
    gaussianPyramid.resize(NOCTAVES * OCTAVE_GAUSSIAN_IMAGES);  // пирамида состоит из NOCTAVES октав,
                                                                // каждая октава состоит из OCTAVE_GAUSSIAN_IMAGES разматых изображений
                                                                // Фишка в том, что в октаве номер i размывается исходное изображение, уменьшенное в 2^i раз
    // То есть итого пирамида такая: 
    // в октаве номер 0: идут OCTAVE_GAUSSIAN_IMAGES штук картинок (размер которых = размеру исходной imgOrg): сама исходная картинка, затем исходная картинка, размытая 1 раз, потом размыттая 2 раза и тд...
    // затем начинается следующая октава - в октаве 1: снова OCTAVE_GAUSSIAN_IMAGES  штук картинок (размер которых = 1/2 размера исходной imgOrg): исходная картинка (уменьшенная в 2 раза, но не размытая), затем её размытия...
    // ...
    // То есть каждая октава начинается с неразмытой, но уменьшенной исходной картинки, а потом в октаве идут её размытия
    ...
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
        if (octave == 0) {
            int layer = 0;  // номер слоя (номер картинки) внутри одной октавы
            gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + layer] = imgOrg.clone();  // 0-ая (layer=0) картинка 0-ой октавы - это исходная картинка
        } else {
            int layer = 0;
            ...
            gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + layer] = img;  // 0-ая картинка окатвы номер octave - это исходная картинка, уменьшенная в 2^octave раз
        }
        ...
        for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {  // теперь внутри октавы номер=octave проходимся по слоям layer - на каждом слое - всё более сильное размытие 0-ой картинки октавы
            ...
            // !!! DONE TODO - переделываем на размытие исходной картинки !!! -> после этого можно раскомментировать #pragma omp ... для параллельности
            double sigmaCur  = INITIAL_IMG_SIGMA * pow(2.0, octave) * pow(k, layer);
            double sigma = sqrt(sigmaCur * sigmaCur - INITIAL_IMG_SIGMA * INITIAL_IMG_SIGMA);  // сигма относительной первой картинки

            cv::Mat imgLayer = gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES].clone();  // берём первую картинку октавы

            cv::Size automaticKernelSize = cv::Size(0, 0);
            cv::GaussianBlur(imgLayer, imgLayer, automaticKernelSize, sigma, sigma);  // размываем

            gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + layer] = imgLayer;
        }
    }
    // короче смысл в том, что ОКТАВА - это как бы размер картинки, а СЛОЙ (layer) - это степень размытия картинки с данной октавы

    // далее просто для отладки сохраняем пирамиду:
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
    ...
    // для каждой октавы DOG хранит (OCTAVE_GAUSSIAN_IMAGES-1) = OCTAVE_DOG_IMAGES картинок = разниц между картинками с соседних слоёв пирамиды гауссианов (для той же окатвы)
    ...
    for (ptrdiff_t octave = 0; octave < NOCTAVES; ++octave) {
        for (size_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
            ...
            cv::Mat imgPrevGaussian = gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + prevLayer];  // две картинки с соседних слоёв одной октавы
            ...
            for (size_t j = 0; j < imgCurDoG.rows; ++j) {
                for (size_t i = 0; i < imgCurDoG.cols; ++i) {
                    imgCurDoG.at<float>(j, i) = imgCurGaussian.at<float>(j, i) - imgPrevGaussian.at<float>(j, i);  // считаем попиксельную разницу!
                }
            }
            int dogLayer = layer - 1;
            DoGPyramid[octave * OCTAVE_DOG_IMAGES + dogLayer] = imgCurDoG;  // записываем разницу
        }
    }
    ...
    // DONE TODO - потому что мы хотим, что финальный экстремум захватывал всю октаву:
    // [цитата из lowe04] We must produce s + 3 images in the stack of blurred images for each octave, so that final extrema detection covers a complete octave
...
namespace {
    // есть значения параболы x0, x1, x2 в 3 точках, функция выдаёт поправку к центральной точке, чтобы получить экстремум параболы
    ...
    //#pragma omp parallel // запустили каждый вычислительный поток процессора
    {
        ...
        for (size_t octave = 0; octave < NOCTAVES; ++octave) {
            ...
            for (size_t layer = 1; layer + 1 < OCTAVE_DOG_IMAGES; ++layer) {
                ...
                const cv::Mat DoGs[3] = {prev, cur, next};  // берём три слоя DOG подряд

                // теперь каждый поток обработает свой кусок картинки 
                //#pragma omp for
                for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
                    for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
                        bool is_max = true;  // изначально считаем, что точка - и минимум, и максимум
                        bool is_min = true;
                        float center = DoGs[1].at<float>(j, i);  // рассматриваем точку на центральном (из трех слоев) слое - перебираем все такие точки, перебирая i,j
                        for (int dz = -1; dz <= 1 && (is_min || is_max); ++dz) {  // далее проверяем, что эта точка - экстремум среди соседей по своему слою, а также по предыдущему и следующему слоям
                        for (int dy = -1; dy <= 1 && (is_min || is_max); ++dy) {  // (как только поймем, что точка ниминимум, ни максимум -> сразу выйдем из цикла, тк is_min и is_max false)
                        ...
                        // DONE TODO:    
                            if (dz == 0 && dy == 0 && dx == 0)  // с самим собой точку не сравниваем
                                continue;
                            if (DoGs[1].at<float>(j, i) <= DoGs[1+dz].at<float>(j+dy, i+dx) && is_min)  // проверка, что точка всё ещё минимум
                                is_max = false;
                            else if (DoGs[1].at<float>(j, i) >= DoGs[1+dz].at<float>(j+dy, i+dx) && is_max)  // то всё ещё максимум
                                is_min = false;
                            else {
                                is_min = false;
                                is_max = false;
                        ...
                        if (!is_extremum)  // если точка не экстремум -> просто ищем дальше!
                            continue; // очередной элемент cascade filtering, если не экстремум - сразу заканчиваем обработку этого пикселя

                        // 4 Accurate keypoint localization - тут уточняем экстремум дробными пикселями, аппроксимиру параболой например
                        cv::KeyPoint kp;  // СОЗДАЁМ КЛЮЧЕВУЮ ТОЧКУ!
                        float dx = 0.0f;
                        float dy = 0.0f;
                        float dvalue = 0.0f;
...
#if SUBPIXEL_FITTING_ENABLE // такие тумблеры включающие/выключающие очередное улучшение алгоритма позволяют оценить какой вклад эта фича вносит в качество результата если в рамках уже готового алгоритма попробовать ее включить/выключить
                        {
                            ...
                            dvalue = parabolaFitting(DoGs[1-1].at<float>(j, i), DoGs[1].at<float>(j, i), DoGs[1+1].at<float>(j, i));
                        }
#endif
                        ...
                        if (contrast < contrast_threshold / OCTAVE_NLAYERS) // TODO почему порог контрастности должен уменьшаться при увеличении числа слоев в октаве?
                            continue;  // если контрастность < трэшхолда, то тоже откидываем точку и ищем дальше

                        // !!! задаём координаты ключевой точки !!! (так как мы картинку в 2 раза уменьшеали,
                        // приходится восстанавливать координаты на исходной картинке через ..._downscale) - в общем случае
                        // координаты задаются дробными (с учётом субпиксельного уточнения dx, dy)
                        kp.pt = cv::Point2f((i + 0.5 + dx) * octave_downscale, (j + 0.5 + dy) * octave_downscale);

                        kp.response = fabs(contrast);  // задаём отклик (контрастность) ключевой точки!

                        const double k = pow(2.0, 1.0 / OCTAVE_NLAYERS); // [lowe04] k = 2^{1/s} а у нас s=OCTAVE_NLAYERS
                        double sigmaCur = INITIAL_IMG_SIGMA * pow(2.0, octave) * pow(k, layer);
                        kp.size = 2.0 * sigmaCur * 5.0;  // задаём размер ключевой точки!!! - для этого:
                                                         // pow(2.0, octave) - увеличивает его, так как на октаве=octave рассматривается исходное изображние, уменьшенное в 2^octve раз -> с помощью этого множителя считаем размер на исходной картинке
                                                         // pow(k, layer); - чем больше размытие (больше layer), тем больше ключевая точка -> увеличиваем размер

                        // 5 Orientation assignment - тут определяем размер ключевой точки!
                        cv::Mat img = gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + layer];
                        std::vector<float> votes;
                        float biggestVote;
                        int oriRadius = (int) (ORIENTATION_WINDOW_R * (1.0 + k * (layer - 1)));
                        if (!buildLocalOrientationHists(img, i, j, oriRadius, votes, biggestVote))  // по точке j,i определяем самое популярное направление biggestVote
                            continue;

                        for (size_t bin = 0; bin < ORIENTATION_NHISTS; ++bin) {  // тут мы сделаем несколько копий точки kp с её самыми попуялрными направлениями
                            float prevValue = votes[(bin + ORIENTATION_NHISTS - 1) % ORIENTATION_NHISTS];
                            float value = votes[bin];  // берём три соседних направлния (с учётом цикличности направлений - поэтому по модулю берём)
                            float nextValue = votes[(bin + 1) % ORIENTATION_NHISTS];
                            if (value > prevValue && value > nextValue && votes[bin] > biggestVote * ORIENTATION_VOTES_PEAK_RATIO) {  // если направлние value - больше соседей и не сильно меньше Максимального
                                // TODO добавьте уточнение угла наклона - может помочь определенная выше функция parabolaFitting(float x0, float x1, float x2)
                                // DONE TODO:
                                float shift = bin + parabolaFitting(prevValue, value, nextValue);  // уточняем направление, аппроксимируя параболой
                                if (shift >= ORIENTATION_NHISTS)
                                    shift -= ORIENTATION_NHISTS;
                                else if (shift < 0)
                                    shift += ORIENTATION_NHISTS;
                                kp.angle = (shift) * (360.0 / ORIENTATION_NHISTS);  // учтанавливаем направление для точки
                                ...
                                thread_points.push_back(kp);  // добавляем точку (её копию) в массив
                                thread_descriptors.push_back(descriptor);  // и её дискриптор
                            }
                        }
                    }
                }
...
double getMagnitude(const cv::Mat &img, size_t x, size_t y) {
    return pow(img.at<float>(y, x + 1) - img.at<float>(y, x - 1), 2) + pow(img.at<float>(y + 1, x) - img.at<float>(y - 1, x), 2);
}

double getOrientation(const cv::Mat &img, size_t x, size_t y) {
    return atan2((img.at<float>(y + 1, x) - img.at<float>(y - 1, x)), (img.at<float>(y, x + 1) - img.at<float>(y, x - 1))); 
}
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i, size_t j, size_t radius,
    ...
    for (size_t y = j - radius + 1; y < j + radius; ++y) {
        for (size_t x = i - radius + 1; x < i + radius; ++x) {
            ...
            // DONE TODO:
            double magnitude = getMagnitude(img, x, y);  // считаем длину градиента
            
            // orientation == theta
            // atan( (L(x, y + 1) − L(x, y − 1)) / (L(x + 1, y) − L(x − 1, y)) )
            // double orientation = TODO // подсказка - используйте atan2(dy, dx)
            // DONE TODO:
            double orientation =  getOrientation(img, x, y);  // и направление градиента в этой точке
            ...
            // DONE TODO:
            size_t bin = static_cast<size_t>(floor(orientation * ORIENTATION_NHISTS / 360));  // номер корзины (= номер дискретного направления), куда попадает это направление
            rassert(bin < ORIENTATION_NHISTS, 361236315613);
            
            //sum[bin] += magnitude;  // прибавляем в данную корзину длину градиента

            // TODO может быть сгладить получившиеся гистограммы улучшит результат? 
            // DONE TODE:
            int SR = 1;  // радиус, в котором сглаживаем
            std::vector<float> smooth;
            smooth.resize(SR * 2 + 1);
            smooth[SR] = magnitude;
            for(size_t i = 1; i < SR + 1; i ++) {
                smooth[SR - i] = magnitude * pow(2, -2.0 * i);
                smooth[SR + i] = magnitude * pow(2, -2.0 * i);
            }
            double norm2 = 0;
            for (float f: smooth)
                norm2 += f * (double)1.0 * f;
            // раскладываем в корзины
            sum[bin] += smooth[SR] / sqrt(norm2) * magnitude;  // нормализуем, чтобы норма=magnitude
        }
    }

    for (size_t bin = 0; bin < ORIENTATION_NHISTS; ++bin) {
        votes[bin] = sum[bin];  // получаем кол-во градиентов в каждом направлении 
        biggestVote = std::max(biggestVote, sum[bin]);  // ищем максимальное направление
    }
...
// для точки с координатами px,py и углом angle строим дискриптор
...
//                            cv::Point2f shift(((-DESCRIPTOR_SIZE/2.0 + hsti) * DESCRIPTOR_SAMPLES_N + smpi) * smpW, TODO);
                            cv::Point2f shift(((-DESCRIPTOR_SIZE/2.0 + hsti) * DESCRIPTOR_SAMPLES_N + smpi) * smpW,
                                              ((-DESCRIPTOR_SIZE/2.0 + hstj) * DESCRIPTOR_SAMPLES_N + smpj) * smpW);  // DONE TODE - получился сдвиг относительно центра гистограмм в центральный пискель замера - сдвиг в пикселях
                            std::vector<cv::Point2f> shiftInVector(1, shift);  // вектор из одного элмента - shift
                            cv::transform(shiftInVector, shiftInVector, relativeShiftRotation); // преобразуем относительный сдвиг с учетом ориентации ключевой точки - поворачиваем ёё обратно на угол
                            shift = shiftInVector[0];  // из вектор получаем преобразованный свдиг

                            int x = (int) (px + shift.x);
                            int y = (int) (py + shift.y);  // DONE TODO; -> координаты 

                            if (y - 1 < 0 || y + 1 > img.rows || x - 1 < 0 || x + 1 > img.cols)
                                return false;

                            double magnitude = getMagnitude(img, x, y);   // DONE TODO

                            double orientation = getOrientation(img, x, y); // atan2(TODO); DONE TODE
                            orientation = orientation * 180.0 / M_PI;
                            orientation = (orientation + 90.0);
                            if (orientation <  0.0)   orientation += 360.0;
                            if (orientation >= 360.0) orientation -= 360.0;
                            ...
                            size_t bin = static_cast<size_t>(floor(orientation * DESCRIPTOR_NBINS / 360));  // DONE TODO;
            ...
            // DONE TODO - нормализация  ??? странно, но это снизило качество на тестах...
            double length = 0;
            for (float f: descriptor) {
                length += f * (double)1.0 * f;
            }
            float norm = sqrt(length);  // евклидова норма
