...
#pragma omp for
#endif
                for (ptrdiff_t j = 1; j + 1 < cur.rows; ++j) {
                    for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
                        ...
                        for (int dx = -1; dx <= 1 && (is_min || is_max); ++dx) {
                            ...
                            if (is_min && DoGs[1 + dz].at<float>(j + dy, i + dx) <= center) {
                                is_min = false;
                            }
                            if (is_max && DoGs[1 + dz].at<float>(j + dy, i + dx) >= center) {
                                is_max = false;
                            }
                        }
                        }
