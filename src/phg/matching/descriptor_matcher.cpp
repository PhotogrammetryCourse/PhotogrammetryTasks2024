    ...
    )
{
    ...
    {
        if(matches[i].size() < 1 || matches[i][0].distance < matches[i][1].distance * MATCHER_THRESHOLD)
        {
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (int i = 0; i < n_matches; i++)
    {
        ...
        for (int j = 0; j < total_neighbours; j++)
        {
            ...
            {
                nearest_neighbours.insert(indices_query.at<int>(i, j));
            }
        }
        int valid = 0;
        for (int j = 0; j < total_neighbours; j++)
        {
            if (
                       (distances2_train.at<float>(i, j) <= radius2_train)
                    && (nearest_neighbours.find(indices_train.at<int>(i, j)) != nearest_neighbours.end())
               )
            {
                ++valid;
            }
        }
        if (valid >= consistent_matches)
        {
