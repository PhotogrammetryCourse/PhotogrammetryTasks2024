                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (size_t i = 0; i < matches.size(); i++) {
        if (matches[i][0].distance < cutoff * cutoff * matches[i][1].distance) {
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    std::shared_ptr<cv::flann::SearchParams> search_params = flannKsTreeSearchParams(20);
    ...
    // оценить радиус поиска для каждой картинкиSS
    ...
    #pragma omp for
    for (int i = 0; i < n_matches; i++) {
        std::set<int> qx, qy, qxy;
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_query.at<float>(i, j) <= radius2_query) {
                qx.insert(indices_query.at<int>(i, j));
                qxy.insert(indices_query.at<int>(i, j));
            }
        }
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_train.at<float>(i, j) <= radius2_train) {
                qy.insert(indices_train.at<int>(i, j));
                qxy.insert(indices_train.at<int>(i, j));
            }
        }

        if (qxy.size() + consistent_matches <= qx.size() + qy.size()) {
            #pragma omp critical
