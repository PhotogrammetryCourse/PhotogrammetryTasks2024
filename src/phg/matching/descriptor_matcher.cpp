                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    // метч остается, если левое и правое множества первых total_neighbors соседей в радиусах
    // поиска(radius2_query, radius2_train) имеют как минимум consistent_matches общих элементов
    // TODO заполнить filtered_matches

    for (int match_index = 0; match_index < n_matches; ++match_index) {
        std::set<int> query_neighbours;
        std::set<int> train_neighbours;

        for (int nn_index = 0; nn_index < total_neighbours; ++nn_index) {
            if (distances2_query.at<float>(match_index, nn_index) < radius2_query) {
                query_neighbours.insert(indices_query.at<int>(match_index, nn_index));
            }
        }

        for (int nn_index = 0; nn_index < total_neighbours; ++nn_index) {
            if (distances2_train.at<float>(match_index, nn_index) < radius2_train) {
                train_neighbours.insert(indices_train.at<int>(match_index, nn_index));
            }
        }
        int intersection_size = 0;
        for (auto nn_index : query_neighbours) {
            if (train_neighbours.find(nn_index) != train_neighbours.end()) {
                intersection_size++;
            }
        }
        if (intersection_size >= consistent_matches) {
            filtered_matches.push_back(matches[match_index]);
        }
    }
}
