...
vector3d unproject(const vector3d &pixel, const phg::Calibration &calibration, const matrix34d &PtoWorld) {
    ...
    rassert(fabs(local_point[2] - 1.0) < 1e-8, 456734645565);
    local_point *= depth;
...
#pragma omp parallel for schedule(dynamic, 1)
        for (ptrdiff_t j = 0; j < height; ++j) {
            for (ptrdiff_t i = (j + chessboard_pattern_step) % 2; i < width; i += 2) {
                ...
                std::vector<size_t> indices(hypos_cost.size());
                std::iota(indices.begin(), indices.end(), 0);
                std::sort(indices.begin(), indices.end(), [&hypos_cost](size_t i, size_t j) {
                    return hypos_cost[i] < hypos_cost[j];
                });
                indices.resize(std::min<size_t>(indices.size(), 8));
...
float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d &global_normal, size_t neighb_cam) {
    ...
    for (ptrdiff_t dj = -COST_PATCH_RADIUS; dj <= COST_PATCH_RADIUS; ++dj) {
        for (ptrdiff_t di = -COST_PATCH_RADIUS; di <= COST_PATCH_RADIUS; ++di) {
            ...
            if (u < 0 || v < 0 || u >= cameras_imgs_grey[neighb_cam].cols - 1 || v >= cameras_imgs_grey[neighb_cam].rows - 1)
                return NO_COST;

            float i00 = cameras_imgs_grey[neighb_cam].at<unsigned char>(v + 0, u + 0) / 255.0f;
            float i01 = cameras_imgs_grey[neighb_cam].at<unsigned char>(v + 1, u + 0) / 255.0f;
            float i10 = cameras_imgs_grey[neighb_cam].at<unsigned char>(v + 0, u + 1) / 255.0f;
            float i11 = cameras_imgs_grey[neighb_cam].at<unsigned char>(v + 1, u + 1) / 255.0f;
            float i0 = (i00 * (x - (u + 0.5)) + i10 * ((u + 1.5) - x));
            float i1 = (i01 * (x - (u + 0.5)) + i11 * ((u + 1.5) - x));
            float intensity = (i0 * (y - (v + 0.5)) + i1 * ((v + 1.5) - y));
    ...
    float zncc = correlation / sqrt(variance0 * variance1);
    if (std::isnan(zncc))
        return NO_COST;
...
float PMDepthMapsBuilder::avgCost(std::vector<float> &costs) {
    ...
    float cost_w = 0;
    float cost_sum = 0;
    for (size_t k = 0; k < costs.size() && k < COSTS_BEST_K_LIMIT && costs[k] <= best_cost * COSTS_K_RATIO; k++) {
        cost_sum += costs[k];
        cost_w += 1;
