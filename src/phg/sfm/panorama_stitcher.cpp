...
)
{
...

    {
        ...
        Hs[root_index] = cv::Mat::eye(3, 3, CV_64F);
        std::queue<int> q;
        q.push(root_index);
        while (!q.empty())
        {
            ...
            for (int i = 0; i < parent.size(); ++i)
            {
                if (parent.at(i) == current_root)
                {
                    q.push(i);
                    Hs[i] = Hs[current_root] * homography_builder(imgs[i], imgs[current_root]);
                }
            }
        }
    }
