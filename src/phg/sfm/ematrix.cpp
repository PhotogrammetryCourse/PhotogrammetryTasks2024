...
namespace {
    ...
    void ensureSpectralProperty(matrix3d &Ecv) {
        ...
        auto D = Eigen::Vector3d({svd.singularValues()[0], svd.singularValues()[0], 0}).asDiagonal();
...
namespace {
    ...
    bool depthTest(const vector2d &m0, const vector2d &m1, const phg::Calibration &calib0,
                   const phg::Calibration &calib1, const matrix34d &P0, const matrix34d &P1) {
...
void phg::decomposeEMatrix(cv::Matx34d &P0, cv::Matx34d &P1, const cv::Matx33d &Ecv, const std::vector<cv::Vec2d> &m0,
