...
cv::Point2d phg::transformPoint(const cv::Point2d &pt, const cv::Mat &T) {
    auto pt3 = cv::Mat(3, 1, CV_64FC1);
    pt3.at<double>(0, 0) = pt.x;
    pt3.at<double>(1, 0) = pt.y;
    pt3.at<double>(2, 0) = 1;
    rassert(pt3.at<double>(0, 0) == pt.x, 9090909);
    cv::Mat res = T * pt3;
    rassert(res.rows == 3 && res.cols == 1, 97979797);
    double c = res.at<double>(2, 0);
    return cv::Point2d(res.at<double>(0, 0) / c, res.at<double>(1, 0) / c);
}
