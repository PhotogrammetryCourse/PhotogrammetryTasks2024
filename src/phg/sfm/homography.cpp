...
namespace {
    ...
    cv::Mat estimateHomography4Points(const cv::Point2f &l0, const cv::Point2f &l1,
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            float moved_to_left = -1.f;
            A.push_back({0.f, 0.f, 0.f, -w1 * x0, -w1 * y0, -w1 * w0, y1 * x0, y1 * y0, moved_to_left * y1 * w0});
            A.push_back({w1 * x0, w1 * y0, w1 * w0, 0.f, 0.f, 0.f, -x1 * x0, -x1 * y0, moved_to_left * -x1 * w0});
        }
//        assert(A.size() == 8;)
        int res = gauss(A, H);
        if (res == 0) {
...
//
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            cv::Mat H = estimateHomography4Points(points_lhs[sample[0]], points_lhs[sample[1]], points_lhs[sample[2]],
                                                  points_lhs[sample[3]],
                                                  points_rhs[sample[0]], points_rhs[sample[1]], points_rhs[sample[2]],
                                                  points_rhs[sample[3]]);
...
cv::Point2d phg::transformPoint(const cv::Point2d &pt, const cv::Mat &T) {
    auto point_matrix = cv::Mat(3, 1, CV_64FC1);
    point_matrix.at<double>(0, 0) = pt.x;
    point_matrix.at<double>(1, 0) = pt.y;
    point_matrix.at<double>(2, 0) = 1.0;
    cv::Mat result = T * point_matrix;
    assert(result.rows == 3 && result.cols == 1);
    auto w = result.at<double>(2, 0);
    auto x = result.at<double>(0, 0) / w;
    auto y = result.at<double>(1, 0) / w;
