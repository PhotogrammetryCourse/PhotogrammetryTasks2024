...
//            std::cout << "gauss: unique solution found" << std::endl;
    ...
    cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs)
    {
        ...
        const float inliers_ratio = 0.5;
        const float success_probability = 0.995;
...
{
    cv::Vec3d point(pt.x, pt.y, 1.0);
    cv::Mat result = T * point;
    return cv::Point2f(result.at<double>(0, 0) / result.at<double>(2, 0), result.at<double>(1, 0) / result.at<double>(2, 0));
}
